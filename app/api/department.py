...
@exception_handler
async def get_all(current_user=Depends(get_current_user)) -> list[DepartmentBase]:
    async with unit_of_work() as uow:
...
async def update_by_id(
    ...
    async with unit_of_work() as uow:
        return await DepartmentService(uow.session).update_department_with_relations(department_id, department)
