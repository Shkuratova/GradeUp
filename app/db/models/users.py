...
class User(Base):
    ...
    managed_department_id: Mapped[int | None] = mapped_column(ForeignKey("departments.id"))
    managed_division_id: Mapped[int | None] = mapped_column(ForeignKey("divisions.id"))

    role: Mapped["Role"] = relationship(back_populates="users")
    department: Mapped["Department"] = relationship(back_populates="users", foreign_keys=[department_id])

    managed_department: Mapped["Department"] = relationship(back_populates="supervisor", foreign_keys=[managed_department_id])
    managed_division: Mapped["Division"] = relationship(back_populates="supervisor", foreign_keys=[managed_division_id])
